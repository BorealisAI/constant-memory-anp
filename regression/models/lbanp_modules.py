...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if norm_first:
            self.dataset_self_attn = PreNorm(self.d_model, Attention(
                self.d_model, heads=nhead, dim_head=self.d_model // nhead, dropout=dropout))
            self.ff = PreNorm(self.d_model, FeedForward(
                self.d_model, dim_feedforward=dim_feedforward, dropout=dropout))
        else:
            self.dataset_self_attn = PostNorm(self.d_model, Attention(
                self.d_model, heads=nhead, dim_head=self.d_model // nhead, dropout=dropout))
            self.ff = PostNorm(self.d_model, FeedForward(
                self.d_model, dim_feedforward=dim_feedforward, dropout=dropout))
