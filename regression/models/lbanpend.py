...
class LBANPEND(LBANP):
    ...
    def __init__(
        ...
        self.std_latents = nn.Parameter(torch.randn(
            num_latents, self.latent_dim), requires_grad=True)

        std_encoder_layer = LBANPEncoderLayer(
            d_model, nhead, dim_feedforward, dropout, norm_first=norm_first, batch_first=True)
        self.std_encoder = LBANPEncoder(std_encoder_layer, num_std_layers)

        std_decoder_layer = LBANPDecoderLayer(
            d_model, nhead, dim_feedforward, dropout, norm_first=norm_first, batch_first=True)
    ...
    def decode(self, out_encoder, batch_size, dim_y, num_target):
        ...
        out_std_context_encodings = self.std_encoder(
            out_encoder, self.std_latents)

        out_std_decoder = self.std_decoder(
            out_encoder, out_std_context_encodings)
    ...
    def predict(self, xc, yc, xt, num_samples=50, return_samples=False):
        ...
        batch.yc = yc
        batch.xt = xt
        batch.yt = torch.zeros(
            (xt.shape[0], xt.shape[1], yc.shape[2]), device='cuda')
